...
| **Forward EN SCROLLÉ** | Nouvelle vue à scrollTop=0 ; **la vue sortante reste figée à son offset** pendant le fade (pas de flash jump-to-top) ; header cross-fade propre (pas de titre dupliqué/téléporté). | useViewTransition.js:153-191 |
| **Back AVEC restore (T>0)** | Vue d'arrivée affichée **déjà à l'offset T**, sans animation scroll-from-top ; header aligné ; après le fade `scrollTop == T` ; pendingScrollRestore vidé. **(le scénario raison-d'être de toute la cicatrice)** | useViewTransition.js:216-250, Modal.vue:155-212 |
  ...
  .modal-wrapper          (inchangé)
    ...
    .modal-shell          [NOUVEAU rôle, ex-.modal-container] overflow:hidden, radius, glass ::before,
      ...
      .modal-clip         [NOUVEAU] overflow:hidden ; PORTE la height animée
                           (transition: height var(--transition-spring-slow)) ; flex:0 0 auto.
...
|---|---|---|
| **Hauteur visible (morph)** | `.modal-clip` | `height` animée par ressort CSS (`--transition-spring-slow`), pilotée par un ref réactif `clipHeight`. Peut overshooter sans danger : ça ne fait que sur-révéler/sur-masquer quelques px d'un scroller déjà posé. |
...
3. **Forward-scrolled : reset scrollTop=0 sur scroller partagé.** Les deux vues sont dans la même cellule grid pendant le fade ; écrire `scrollTop=0` en `onEnter` tirerait la sortante en haut. *Choix : transform `translateY(-oldScrollTop)` sur la sortante (mini-cycle de style) OU différer le `scrollTop=0` à `onAfterLeave` (1 frame). À départager par eyeball Pi/iPhone — les deux sont bien plus simples que clone+CSS-offset.*
4. **24px bounceMargin (Modal.vue:69).** Devenu cosmétique. *Le réduire à ~8px ou le garder ? Réglage post-revue visuelle.*
5. **ResizeObserver écrit-il `scrollerHeight` à chaque changement, ou seulement hors-nav ?** Comme la nav écrit synchronement et idempotemment la même cible, laisser l'observer écrire aussi est inoffensif — *confirmer qu'il n'y a pas de boucle de feedback (observer sur contentInner, pas sur le scroller).*
6. **AudioSourceLayout : `prepareNavigation` devient-il no-op** une fois clone/offset/pin supprimés ? Si oui, le retirer de ce consommateur (AudioSourceLayout.vue:210-213) pour ne pas laisser de code mort. *À vérifier à l'étape 5.*
7. **`modalDebug` (modalDebugLog/Trace).** Tout le sous-système de trace scroll disparaît avec la cicatrice. *Confirmer qu'on supprime `services/modalDebug.js` et son allowlist console (cf. allowlist `services/logger.js`) plutôt que de le laisser orphelin.*
8. **Interaction scale de fermeture × scroller à hauteur explicite.** La hauteur explicite du scroller est indépendante du `scale` de close (Modal.vue:363-365) — *vérifier que le scale-down ne révèle pas d'overflow aux coins arrondis.*
