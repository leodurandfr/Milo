...
# =============================================================================
# GET /api/audio/commands — the table clients used to discover by probing
# =============================================================================

class TestCommandsListing:
    """The command table is served, and it is DERIVED from COMMANDS.

    When this fails, every client is back to sending names and reading 400s to
    find out what a source accepts — a survey that only ever finds what its
    author thought to try. Milo-iOS' missed `playpause`, `seek`, `set_speed`,
    `set_shuffle`, `eject` and `disconnect` across six sources.
    """
    ...
    def test_a_source_that_takes_nothing_is_listed_empty_not_omitted(self, client, source):
        """The receivers (AirPlay, DLNA, Qobuz, Mac) declare no command. Omitting
        them would make "takes nothing" indistinguishable from "not registered",
        and a client cannot tell those apart from the outside."""
        source.COMMANDS = {}
