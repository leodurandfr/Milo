...
class TestRoutingEnv:
    async def test_regenerate_writes_multiroom_mode(self, env_paths):
        await RoutingEnv.regenerate(True)
        content = _read(env_paths["routing"])
        assert "MILO_MODE=multiroom" in content

    async def test_regenerate_writes_direct_mode(self, env_paths):
        await RoutingEnv.regenerate(False)
        content = _read(env_paths["routing"])
        assert "MILO_MODE=direct" in content

    async def test_regenerate_sets_os_environ(self, env_paths, monkeypatch):
        monkeypatch.setenv("MILO_MODE", "stale_value")
        await RoutingEnv.regenerate(True)
        assert os.environ["MILO_MODE"] == "multiroom"

    async def test_regenerate_does_not_read_os_environ_as_truth(self, env_paths, monkeypatch):
    ...
    async def test_regenerate_idempotent(self, env_paths):
        await RoutingEnv.regenerate(True)
    ...
    async def test_regenerate_only_writes_milo_mode(self, env_paths):
...
class TestMacEnv:
    async def test_regenerate_writes_settings_values(self, env_paths):
    ...
    async def test_regenerate_falls_back_to_defaults_when_none(self, env_paths):
    ...
    async def test_regenerate_falls_back_to_defaults_when_empty(self, env_paths):
        await MacEnv.regenerate({})
        content = _read(env_paths["mac"])
        assert f"ROC_TARGET_LATENCY={DEFAULT_ROC_CONFIG['target_latency_ms']}ms" in content
        assert f"ROC_LATENCY_PROFILE={DEFAULT_ROC_CONFIG['latency_profile']}" in content
    ...
    async def test_regenerate_ignores_os_environ(self, env_paths, monkeypatch):
    ...
    async def test_regenerate_clamps_target_latency_high(self, env_paths):
        await MacEnv.regenerate({"target_latency_ms": 9999})
        content = _read(env_paths["mac"])
        assert "ROC_TARGET_LATENCY=500ms" in content

    async def test_regenerate_clamps_target_latency_low(self, env_paths):
        await MacEnv.regenerate({"target_latency_ms": 1})
        content = _read(env_paths["mac"])
        assert "ROC_TARGET_LATENCY=20ms" in content

    async def test_regenerate_rejects_invalid_profile(self, env_paths):
        await MacEnv.regenerate({"latency_profile": "bogus"})
        content = _read(env_paths["mac"])
        assert f"ROC_LATENCY_PROFILE={DEFAULT_ROC_CONFIG['latency_profile']}" in content

    async def test_regenerate_rejects_invalid_frame_length(self, env_paths):
        await MacEnv.regenerate({"frame_length_ms": 99})
        content = _read(env_paths["mac"])
        assert f"ROC_FRAME_LENGTH={DEFAULT_ROC_CONFIG['frame_length_ms']}ms" in content

    async def test_regenerate_does_not_touch_routing_env(self, env_paths):
        """Regression for the original Spotify bug: changing ROC settings must
        NOT rewrite routing.env / flip MILO_MODE."""
        await RoutingEnv.regenerate(True)  # routing.env exists, MILO_MODE=multiroom
        before = _read(env_paths["routing"])

        await MacEnv.regenerate({"target_latency_ms": 123})
...
class TestSnapclientEnv:
    async def test_regenerate_writes_provided_values(self, env_paths):
        await SnapclientEnv.regenerate(120, 6)
    ...
    async def test_regenerate_ignores_os_environ(self, env_paths, monkeypatch):
        monkeypatch.setenv("MILO_MODE", "direct")
        await SnapclientEnv.regenerate(90, 3)
        content = _read(env_paths["snapclient"])
        assert "MILO_SNAPCLIENT_BUFFER_TIME=90" in content
        assert "MILO_SNAPCLIENT_FRAGMENTS=3" in content

    async def test_regenerate_clamps_buffer_to_the_declared_range(self, env_paths):
    ...
    async def test_regenerate_does_not_touch_routing_env(self, env_paths):
        ...
        await SnapclientEnv.regenerate(50, None)
    ...
    async def test_regenerate_idempotent(self, env_paths):
        await SnapclientEnv.regenerate(100, 5)
...
class TestEnvIsolation:
    async def test_each_regenerate_writes_only_its_file(self, env_paths):
        ...
        await MacEnv.regenerate({"target_latency_ms": 250})
        ...
        await SnapclientEnv.regenerate(150, None)
        assert _read(env_paths["routing"]) == routing_after_toggle
...
class TestRegenerateEnvFilesBootstrap:
    ...
    async def test_writes_all_three_files_from_settings(self, env_paths):
        ...
        await service.regenerate_env_files()
    ...
    async def test_writes_defaults_when_settings_missing_keys(self, env_paths):
    ...
    async def test_writes_defaults_when_no_settings_service(self, env_paths):
