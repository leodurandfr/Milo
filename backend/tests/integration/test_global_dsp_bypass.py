# backend/tests/integration/test_global_dsp_bypass.py
"""
Integration tests for Story 4-5: Global DSP Bypass
...
- AC3: Zone propagation for DSP bypass (API-level validation)
...
@pytest.fixture
def connected_dsp_with_effects(mock_settings_service, mock_event_bus, mock_state_machine):
    """Create connected DSP service with EQ, compressor, and loudness enabled"""
...
class TestAC1BypassEffects:
    ...
    @pytest.mark.asyncio
    async def test_bypass_resets_all_eq_bands_to_zero(self, connected_dsp_with_effects, mock_settings_service):
        ...
        with patch.object(connected_dsp_with_effects, '_get_config', new_callable=AsyncMock, return_value=mock_config):
            with patch.object(connected_dsp_with_effects, '_set_config', new_callable=AsyncMock, side_effect=capture_config):
                with patch.object(connected_dsp_with_effects, 'save_current_config', new_callable=AsyncMock):
                    result = await connected_dsp_with_effects.bypass_effects()
    ...
    @pytest.mark.asyncio
    async def test_bypass_disables_compressor(self, connected_dsp_with_effects, mock_settings_service):
    ...
    @pytest.mark.asyncio
    async def test_bypass_disables_loudness(self, connected_dsp_with_effects, mock_settings_service):
    ...
    @pytest.mark.asyncio
    async def test_bypass_broadcasts_effects_bypassed_event(self, connected_dsp_with_effects, mock_state_machine):
...
class TestAC2RestoreEffects:
    """AC2: Toggle enables restore_effects() → restore all DSP from settings"""

    @pytest.mark.asyncio
    async def test_restore_loads_eq_filters_from_settings(self, connected_dsp_with_effects, mock_settings_service):
        """Should restore EQ filters from dsp.filters settings"""
        saved_filters = [
        ...
        with patch.object(connected_dsp_with_effects, '_get_config', new_callable=AsyncMock, return_value=mock_config):
            with patch.object(connected_dsp_with_effects, '_set_config', new_callable=AsyncMock):
                result = await connected_dsp_with_effects.restore_effects()

                assert result is True
                # Verify get_setting was called for dsp.filters
                filter_calls = [c for c in mock_settings_service.get_setting.call_args_list
                               if c[0][0] == "dsp.filters"]
                assert len(filter_calls) >= 1

    @pytest.mark.asyncio
    async def test_restore_loads_compressor_from_settings(self, connected_dsp_with_effects, mock_settings_service):
        """Should restore compressor settings from dsp.compressor"""
        saved_compressor = {
            "enabled": True,
            "threshold": -25,
            "ratio": 6,
        ...
        mock_settings_service.get_setting = AsyncMock(side_effect=lambda key: {
            "dsp.filters": [],
            "dsp.compressor": saved_compressor,
            "dsp.loudness": {"enabled": False}
        }.get(key))
    ...
    @pytest.mark.asyncio
    async def test_restore_loads_loudness_from_settings(self, connected_dsp_with_effects, mock_settings_service):
        ...
        mock_settings_service.get_setting = AsyncMock(side_effect=lambda key: {
            "dsp.filters": [],
            "dsp.compressor": {"enabled": False},
            "dsp.loudness": saved_loudness
        }.get(key))
    ...
    @pytest.mark.asyncio
    async def test_restore_broadcasts_effects_restored_event(self, connected_dsp_with_effects, mock_state_machine, mock_settings_service):
...
class TestAC3ZonePropagation:
    """AC3: DSP bypass/restore propagates to all zone members within 200ms"""

    @pytest.mark.asyncio
    async def test_client_enabled_proxy_route_exists(self):
        """Verify /api/dsp/client/{hostname}/enabled route exists"""
        ...
        assert "/api/dsp/client/{hostname}/enabled" in routes, \
            "Proxy route /api/dsp/client/{hostname}/enabled should exist"

    @pytest.mark.asyncio
    async def test_local_client_enabled_uses_routing_service(self, connected_dsp_with_effects):
        """PUT /api/dsp/client/local/enabled should use routing_service"""
        # This test verifies the route logic handles "local" correctly
        # Full integration would require FastAPI TestClient

        # Verify routing_service.set_dsp_effects_enabled exists and is callable
        ...
        mock_sm.system_state.dsp_effects_enabled = False
        mock_sm.broadcast_event = AsyncMock()
        routing.set_state_machine(mock_sm)

        # Mock camilladsp_service
        mock_dsp = Mock()
        mock_dsp.bypass_effects = AsyncMock(return_value=True)
        mock_dsp.restore_effects = AsyncMock(return_value=True)
        routing.set_camilladsp_service(mock_dsp)

        # Test enabling DSP effects
        result = await routing.set_dsp_effects_enabled(True)
        assert result is True
        mock_dsp.restore_effects.assert_called_once()

    @pytest.mark.asyncio
    async def test_remote_client_enabled_proxies_to_client(self):
        """PUT /api/dsp/client/{hostname}/enabled should proxy to remote client"""
        from backend.api.dsp import create_dsp_router
...
class TestAC4CrossoverIndependence:
    ...
    @pytest.mark.asyncio
    async def test_bypass_preserves_crossover_highpass(self, connected_dsp_with_effects):
    ...
    @pytest.mark.asyncio
    async def test_bypass_preserves_crossover_lowpass(self, connected_dsp_with_effects):
...
class TestAC5SettingsPersistence:
    ...
    @pytest.mark.asyncio
    async def test_bypass_does_not_overwrite_saved_eq_settings(self, connected_dsp_with_effects, mock_settings_service):
        """Bypass should NOT call set_setting for dsp.filters (persist=False)"""
        mock_config = {
        ...
        with patch.object(connected_dsp_with_effects, '_get_config', new_callable=AsyncMock, return_value=mock_config):
            with patch.object(connected_dsp_with_effects, '_set_config', new_callable=AsyncMock):
                with patch.object(connected_dsp_with_effects, 'save_current_config', new_callable=AsyncMock):
                    ...
                    # Verify dsp.filters was NOT saved during bypass
                    filter_calls = [c for c in mock_settings_service.set_setting.call_args_list
                                   if c[0][0] == "dsp.filters"]
                    assert len(filter_calls) == 0, \
                        "set_setting should NOT be called for dsp.filters during bypass"

    @pytest.mark.asyncio
    async def test_bypass_does_not_overwrite_saved_compressor_settings(self, connected_dsp_with_effects, mock_settings_service):
        """Bypass should NOT call set_setting for dsp.compressor (persist=False)"""
        mock_config = {
            "filters": {},
        ...
        with patch.object(connected_dsp_with_effects, '_get_config', new_callable=AsyncMock, return_value=mock_config):
            with patch.object(connected_dsp_with_effects, '_set_config', new_callable=AsyncMock):
                with patch.object(connected_dsp_with_effects, 'save_current_config', new_callable=AsyncMock):
                    ...
                    # Verify dsp.compressor was NOT saved during bypass
                    ...
                    assert len(compressor_calls) == 0, \
                        "set_setting should NOT be called for dsp.compressor during bypass"

    @pytest.mark.asyncio
    async def test_bypass_does_not_overwrite_saved_loudness_settings(self, connected_dsp_with_effects, mock_settings_service):
        """Bypass should NOT call set_setting for dsp.loudness (persist=False)"""
        mock_config = {
        ...
        with patch.object(connected_dsp_with_effects, '_get_config', new_callable=AsyncMock, return_value=mock_config):
            with patch.object(connected_dsp_with_effects, '_set_config', new_callable=AsyncMock):
                with patch.object(connected_dsp_with_effects, 'save_current_config', new_callable=AsyncMock):
                    ...
                    # Verify dsp.loudness was NOT saved during bypass
                    ...
                    assert len(loudness_calls) == 0, \
                        "set_setting should NOT be called for dsp.loudness during bypass"
...
class TestAC6StateSync:
    """AC6: DSP enabled state syncs on reconnection and multiroom mode switch"""

    @pytest.mark.asyncio
    async def test_bypass_fails_when_disconnected(self, disconnected_dsp_service):
        """bypass_effects() should fail gracefully when disconnected"""
        result = await disconnected_dsp_service.bypass_effects()
        assert result is False

    @pytest.mark.asyncio
    async def test_restore_fails_when_disconnected(self, disconnected_dsp_service):
        """restore_effects() should fail gracefully when disconnected"""
        result = await disconnected_dsp_service.restore_effects()
        assert result is False

    @pytest.mark.asyncio
    async def test_routing_service_applies_state_on_init(self):
        """Routing service should apply dsp_effects_enabled on initialization

        This test verifies that during _detect_initial_state(), the routing service
        calls restore_effects() when dsp.effects_enabled is True in settings, or
        ...
        mock_settings.get_setting = AsyncMock(side_effect=lambda key: {
            ...
            "dsp.effects_enabled": True,  # DSP effects enabled in settings
            "dsp.enabled": None,
        }.get(key))
        mock_settings.set_setting = AsyncMock()
        ...
        mock_sm.system_state.dsp_effects_enabled = True  # Match settings
        mock_sm.broadcast_event = AsyncMock()
        routing.set_state_machine(mock_sm)

        # Mock camilladsp_service (must not be connected yet for connect to succeed)
        mock_dsp = Mock()
        mock_dsp.connected = False  # Not connected initially
        mock_dsp.connect = AsyncMock(return_value=True)  # Connect succeeds
        mock_dsp.bypass_effects = AsyncMock(return_value=True)
        mock_dsp.restore_effects = AsyncMock(return_value=True)
        routing.set_camilladsp_service(mock_dsp)
        ...
        # connect() should be called, then restore_effects() since dsp.effects_enabled = True
        mock_dsp.connect.assert_called_once()
        mock_dsp.restore_effects.assert_called_once()
...
class TestDspEnabledAPI:
    """Test /api/dsp/enabled endpoint behavior"""

    @pytest.mark.asyncio
    async def test_get_enabled_returns_routing_state(self):
        """GET /api/dsp/enabled should return routing_service.dsp_effects_enabled"""
        ...
        mock_sm = Mock()
        mock_routing = Mock()
        mock_routing.dsp_effects_enabled = True
        ...
        # Verify route exists
        routes = [r.path for r in router.routes]
        assert "/api/dsp/enabled" in routes

    @pytest.mark.asyncio
    async def test_put_enabled_calls_routing_service(self):
        """PUT /api/dsp/enabled should call routing_service.set_dsp_effects_enabled"""
        from backend.core.multiroom.routing import AudioRoutingService
        ...
        mock_sm._state_lock = asyncio.Lock()
        mock_sm.system_state = Mock()
        mock_sm.system_state.dsp_effects_enabled = True  # Currently enabled
        mock_sm.broadcast_event = AsyncMock()
        routing.set_state_machine(mock_sm)
        ...
        # Test disabling DSP effects (enabled -> disabled)
        result = await routing.set_dsp_effects_enabled(False)

        assert result is True
        mock_dsp.bypass_effects.assert_called_once()
        mock_settings.set_setting.assert_any_call('dsp.effects_enabled', False)
