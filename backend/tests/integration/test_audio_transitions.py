...
from .conftest import WebSocketEventCollector, create_mock_plugin
...
class TestTransitionSequence:
    ...
    async def test_transition_sequence_none_radio_spotify_none(
        self,
        state_machine_with_plugins: AudioStateMachine,
        mock_plugins,
        ...
        - Plugins are correctly started and stopped
        """
        sm = state_machine_with_plugins
        ...
        assert sm.system_state.transitioning is False
        mock_plugins[AudioSource.RADIO].start.assert_called_once()
        ...
        assert sm.system_state.transitioning is False
        mock_plugins[AudioSource.RADIO].stop.assert_called_once()
        mock_plugins[AudioSource.SPOTIFY].start.assert_called_once()
        ...
        assert sm.system_state.active_source == AudioSource.NONE
        assert sm.system_state.transitioning is False
        mock_plugins[AudioSource.SPOTIFY].stop.assert_called_once()
    ...
    async def test_state_consistency_after_transition(
        ...
        - plugin_state reflects active plugin state
        """
        sm = state_machine_with_plugins
    ...
    async def test_transition_to_same_source_is_noop(
        ...
        await sm.transition_to_source(AudioSource.RADIO)
        mock_plugins[AudioSource.RADIO].start.reset_mock()
        ...
        assert sm.system_state.active_source == AudioSource.RADIO
        # Plugin should NOT be started again
        mock_plugins[AudioSource.RADIO].start.assert_not_called()
...
class TestWebSocketEvents:
    ...
    async def test_websocket_events_during_transition(
        ...
        - Events have correct format: {category, type, source, data}
        """
        sm = state_machine_with_plugins
    ...
    async def test_event_format_has_required_fields(
        ...
        for event in websocket_collector.events:
            ...
            assert "source" in event, "Event should have 'source' field"
...
class TestDirectTransition:
    ...
    async def test_direct_transition_between_sources(
        ...
        mock_plugins[AudioSource.RADIO].stop.assert_called()

        # Verify start was called on SPOTIFY
        mock_plugins[AudioSource.SPOTIFY].start.assert_called()
    ...
    async def test_only_one_source_active_at_time(
        ...
        original_radio_stop = mock_plugins[AudioSource.RADIO].stop
        original_spotify_start = mock_plugins[AudioSource.SPOTIFY].start
        ...
        mock_plugins[AudioSource.RADIO].stop = tracked_radio_stop
        mock_plugins[AudioSource.SPOTIFY].start = tracked_spotify_start
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_transition_with_plugin_start_failure(
        self,
        integration_state_machine: AudioStateMachine,
        websocket_collector: WebSocketEventCollector
    ):
        """
        When plugin.start() fails, should rollback to NONE and broadcast error.

        Validates AC5:
        - Plugin.start() fails -> rollback to NONE
        - Error event broadcast
        """
        sm = integration_state_machine

        # Register a failing plugin
        failing_plugin = create_mock_plugin(AudioSource.RADIO, start_success=False)
        sm.register_plugin(AudioSource.RADIO, failing_plugin)
        ...
        assert result is False, "Should fail when plugin.start() fails"
    ...
    async def test_transition_timeout(
        ...
        # Create a plugin that takes too long to start
        slow_plugin = create_mock_plugin(AudioSource.RADIO)

        async def very_slow_start():
            await asyncio.sleep(10)  # Longer than TRANSITION_TIMEOUT (5s)
            return True

        slow_plugin.start = very_slow_start
        sm.register_plugin(AudioSource.RADIO, slow_plugin)
    ...
    @pytest.mark.asyncio
    async def test_emergency_stop_clears_all_plugins(
        self,
        state_machine_with_plugins: AudioStateMachine,
        mock_plugins
    ):
        """
        Emergency stop should attempt to stop all registered plugins.
        """
        sm = state_machine_with_plugins
...
class TestUpdateBuffering:
    ...
    async def test_updates_buffered_during_transition(
        ...
        # Create a plugin that sends an update during start
        ...
        plugin.start = start_with_update
        sm.register_plugin(AudioSource.RADIO, plugin)
