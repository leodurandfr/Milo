...
class TestAPIValidation:
    ...
    def test_equalizer_filter_request_frequency_range(self):
        """EqualizerFilterRequest should accept frequency 20-20000 Hz"""
        from backend.api.models import EqualizerFilterRequest

        # Valid minimum
        f = EqualizerFilterRequest(freq=20, gain=0, q=1.0)
        assert f.freq == 20

        # Valid maximum
        f = EqualizerFilterRequest(freq=20000, gain=0, q=1.0)
        assert f.freq == 20000

    def test_equalizer_filter_request_gain_range(self):
        """EqualizerFilterRequest should accept gain -15 to +15 dB (superset of AC3 -12 to +12)"""
        from backend.api.models import EqualizerFilterRequest

        # Valid minimum (implementation allows -15)
        f = EqualizerFilterRequest(freq=1000, gain=-15, q=1.0)
        assert f.gain == -15
        ...
        f = EqualizerFilterRequest(freq=1000, gain=-12, q=1.0)
        assert f.gain == -12
        f = EqualizerFilterRequest(freq=1000, gain=12, q=1.0)
        assert f.gain == 12

    def test_equalizer_filter_request_q_range(self):
        """EqualizerFilterRequest should accept Q 0.1-10.0"""
        from backend.api.models import EqualizerFilterRequest

        # Valid minimum
        f = EqualizerFilterRequest(freq=1000, gain=0, q=0.1)
        assert f.q == 0.1

        # Valid maximum
        f = EqualizerFilterRequest(freq=1000, gain=0, q=10.0)
        assert f.q == 10.0

    def test_equalizer_filter_request_invalid_frequency_rejected(self):
        """EqualizerFilterRequest should reject out-of-range frequency"""
        from backend.api.models import EqualizerFilterRequest
        from pydantic import ValidationError

        with pytest.raises(ValidationError):
            EqualizerFilterRequest(freq=19, gain=0, q=1.0)  # Below 20 Hz

        with pytest.raises(ValidationError):
            EqualizerFilterRequest(freq=20001, gain=0, q=1.0)  # Above 20000 Hz

    def test_equalizer_filter_request_invalid_gain_rejected(self):
        """EqualizerFilterRequest should reject out-of-range gain"""
        from backend.api.models import EqualizerFilterRequest
        from pydantic import ValidationError

        with pytest.raises(ValidationError):
            EqualizerFilterRequest(freq=1000, gain=-16, q=1.0)  # Below -15 dB

        with pytest.raises(ValidationError):
            EqualizerFilterRequest(freq=1000, gain=16, q=1.0)  # Above +15 dB

    def test_equalizer_filter_request_invalid_q_rejected(self):
        """EqualizerFilterRequest should reject out-of-range Q"""
        from backend.api.models import EqualizerFilterRequest
        from pydantic import ValidationError

        with pytest.raises(ValidationError):
            EqualizerFilterRequest(freq=1000, gain=0, q=0.05)  # Below 0.1

        with pytest.raises(ValidationError):
            EqualizerFilterRequest(freq=1000, gain=0, q=11.0)  # Above 10.0
