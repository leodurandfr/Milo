...
class TestSettingsService:
    ...
    def test_load_settings_file_not_exists(self, service):
        """Test du chargement quand le fichier n'existe pas"""
        settings = service.load_settings()

        # Devrait créer le fichier avec les defaults
        assert os.path.exists(service.settings_file)
        assert settings == service.defaults
        assert service._cache == service.defaults

    def test_load_settings_file_exists(self, service, temp_settings_file):
    ...
    def test_validate_and_merge_language(self, service):
        ...
        # Langue invalide - fallback à french
    ...
    def test_get_setting_simple(self, service):
        """Test de récupération d'une setting simple"""
        service._cache = {'language': 'french'}

        value = service.get_setting('language')

        assert value == 'french'

    def test_get_setting_nested(self, service):
        """Test de récupération d'une setting imbriquée"""
        service._cache = {
            'volume': {'alsa_min': 10, 'alsa_max': 65}
        }

        value = service.get_setting('volume.alsa_min')

        assert value == 10

    def test_get_setting_not_found(self, service):
        """Test de récupération d'une setting inexistante"""
        service._cache = {'language': 'french'}

        value = service.get_setting('nonexistent.key')

        assert value is None

    def test_get_setting_loads_if_no_cache(self, service, temp_settings_file):
    ...
    def test_set_setting_simple(self, service):
        """Test de modification d'une setting simple"""
        service._cache = service.defaults.copy()

        result = service.set_setting('language', 'spanish')
        ...
        saved_value = service.get_setting('language')
        assert saved_value == 'spanish'

    def test_set_setting_nested(self, service):
        """Test de modification d'une setting imbriquée"""
        service._cache = service.defaults.copy()

        result = service.set_setting('volume.alsa_min', 20)

        assert result is True

        # Vérifier que la valeur a été sauvegardée
        saved_value = service.get_setting('volume.alsa_min')
        assert saved_value == 20

    def test_set_setting_create_nested_path_in_existing_section(self, service):
        """Test de création d'un chemin imbriqué dans une section existante"""
        # Initialiser le fichier avec les defaults
        service.save_settings(service.defaults)
        service._cache = None  # Forcer le reload

        # Créer un nouveau chemin dans la section 'volume' qui existe déjà
        result = service.set_setting('volume.new_setting', 123)

        assert result is True

        # Vérifier que le chemin a été créé
        saved_value = service.get_setting('volume.new_setting')
    ...
    def test_migration_display_to_screen(self, service, temp_settings_file):
    ...
    def test_load_settings_error_fallback_to_defaults(self, service):
        ...
        # Devrait fallback aux defaults
        assert settings == service.defaults
        assert service._cache == service.defaults

    def test_save_settings_error_cleanup_temp_file(self, service):
        """Test du nettoyage du fichier temporaire en cas d'erreur"""
        # Mock l'ouverture de fichier pour lever une exception
        with patch('builtins.open', side_effect=Exception('Write error')):
            result = service.save_settings({'language': 'french'})
