...
# is sent for it: see `source_card`.
...
def source_card(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    does not know. Hardly ever sent for `none`: the push service ends the
    session there (`PushService._consider_ending`). The one exception is the
    knock on a session that ended before its token landed
    (`PushService._send_end`) — a card already up, re-announced until the
    token the `end` needs is registered, which is seconds.
