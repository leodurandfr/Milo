...
| … the grey failure, link up and gateway dead ⚠ | The faithful repro of the 2026-09-20 incident, which pulling the cable does **not** reproduce — carrier and DHCP lease both stay valid: `sudo ip neigh replace <gateway> dev eth0 lladdr 00:00:00:00:00:01`, undo with `sudo ip neigh del <gateway> dev eth0`. Here no link event fires at all, so the verdict must be lifted by the hold loop's own re-probe (20 s, then doubling to 120 s) and **not** by a backend restart. Watch for `IPv4 probe could not connect, holding NM's FULL down` followed later by a recovery line | targeted |
