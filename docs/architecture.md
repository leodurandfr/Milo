...
**password-changed** - Empty marker, created by `milo-set-password` and read only for its existence. Absent means the unit still carries the factory password `pi-gen/config` bakes — which is the same on every Milō — so `PUT /api/system/ssh` refuses to start the SSH server, the one remote path that would accept it. Durable: losing it makes the SSH switch inert until a password is set again, which is the safe direction.
