 ...
 *
 * Works with all types of screens (touchscreen or not).
 */
import { onMounted, onUnmounted } from 'vue';
import axios from 'axios';

let lastActivityTime = 0;
const THROTTLE_DELAY = 2000; // 2 seconds minimum between each notification
...
export function useScreenActivity() {
  const notifyActivity = async () => {
    ...
    try {
      ...
      // Silent fail - don't pollute console with these errors
      // Backend already logs errors if necessary
