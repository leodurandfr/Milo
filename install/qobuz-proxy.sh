...
# Write /var/lib/milo/qobuz/config.yaml. Flat single-speaker form → qobuz-proxy
# builds one "local" backend speaker targeting the milo_qobuz ALSA PCM. Auth
# credentials are NOT written here — the OAuth token is cached separately in
...
backend:
  type: "local"
  local:
    device: "milo_qobuz"   # named ALSA PCM → CamillaDSP (direct) / LoopbackDLNA (multiroom)
    buffer_size: 2048

server:
  http_port: 8689
  bind_address: "0.0.0.0"  # phone must reach :8689 over the LAN via mDNS
