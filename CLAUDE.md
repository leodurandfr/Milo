...
**Convergence option (small, future).** Exactly three routes are Milo-Mac-exclusive — `GET /api/settings/volume-limits`, `volume-steps`, `dock-apps` (every other endpoint Milo-Mac calls is already shared with Milō's own frontend). They duplicate fields of `GET /api/settings/bulk` (which the frontend uses) — except `step_mobile_db`, which `/bulk` does not yet return. Pointing Milo-Mac at `/bulk` (after adding `volume_steps` to it) would let those three be deleted. Until then they stay.
