...
| **C. Active player** — UI control, rich metadata | Spotify, Radio, Podcast, CD, Music Library | `source.py` + networking as needed: `websocket.py` (Spotify), full `routes.py`+`data.py`+external API (Radio/Podcast/CD), `models.py`. Music Library is the richest — a catalog-engine split (`navidrome_client.py`, `discovery.py`, `browse.py`, `disc_merge.py`, `storage.py` for USB/SMB/NFS mounting) on top of the usual `routes.py`+`data.py`+`models.py`. | One of the **two** shared players (see below) + custom UI (tracklist/queue/favorites). |
