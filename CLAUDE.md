...
|---|---|---|---|
| **A. Mute receiver** — external control, no rich metadata | Mac | `source.py` + source-specific helpers (`log_patterns.py`). **No `routes.py`** — commands via generic `/api/audio/control/{source}`. | None — rendered by `AudioSourceStatus` (icon + device name). |
