...
**A route or WS event with no caller in `frontend/src/` is NOT necessarily dead** — Milo-Mac may consume it. Before deleting any HTTP route, WS event type, or wire field, grep Milo-Mac first: `gh repo clone leodurandfr/Milo-Mac /tmp/milo-mac -- --depth 1`, then grep `MiloAPIService.swift` (REST) and `WebSocketService.swift` (WS). Removing `/api/settings/{volume-limits,volume-steps,dock-apps}` once broke it.

Milo-Mac couples to the **whole wire contract**: REST paths/methods + request/response keys, and WS events `system`/`source`/`volume`/`equalizer`/`routing`/`settings` (the `full_state` envelope, `multiroom_changed`, `volume_changed` incl. `step_mobile_db`, `settings/{volume_limits,dock_apps}_changed`). **Exception:** it reads WS `metadata` as an opaque dict — no metadata *sub-field* (`uri`, `client_count`, `album_art_height`, `track_position/duration`) is coupled, so an over-emitted metadata sub-field with no `frontend/src/` consumer is safe to drop. **Safe without grepping Milo-Mac:** purely frontend code (Vue components, Pinia stores, frontend Zod schemas).
...
State flow: backend change → `state_machine.broadcast_event()` → WS → Pinia store → reactive UI. Deep dive: [docs/architecture.md](docs/architecture.md).
...
- **Never mutate state directly** — `await state_machine.update_source_state(source, SourceState.ACTIVE, metadata)`, never `state_machine._state.active_source = …`. Transitions are guarded by `_transition_lock`; updates during a transition are buffered and replayed.
- **Broadcast every state change** via `state_machine.broadcast_event(category, type, data)` — never `ws_manager.broadcast_dict()`. Wire format `{category, type, origin, data, timestamp}`; `origin` is read from `data["source"]` (falls back to `category`), so category `"source"` callers **must** put `"source"` in `data`. Categories: `source` (all audio sources — never source-specific ones), `system`, `routing`, `equalizer`, `multiroom`, `volume`, `settings`, `programs`.
- **WS payload contracts** — every payload the frontend consumes SHOULD have a Zod schema in `frontend/src/schemas/ws.js`; handlers consume it via `parsedOn(category, type, schema, handler)` and MUST NOT read `event.data.x` raw. Document the shape in a docstring at each `broadcast_event()` site (kept in sync by review, no codegen). The registry is incremental — only previously-faulty pairs are schematized; others still dispatch raw via `on(...)` until migrated.
...
- **WS events:** handle in Pinia stores, not in components — components react to store state.
