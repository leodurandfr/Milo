...
The FastAPI REST API + WebSocket have a **second consumer beyond `frontend/`: Milo-Mac**, a separate macOS app (repo `github.com/leodurandfr/Milo-Mac`, not in this checkout). It remotely drives a Milō unit — controlling sources, volume, multiroom, EQ and radio, and reading settings — in addition to streaming audio via ROC.

**Dead-code rule — a route or WS event with no caller in `frontend/src/` is NOT necessarily dead.** Milo-Mac may consume it. Before deleting any HTTP route, WS event type, or wire field, grep Milo-Mac first: `gh repo clone leodurandfr/Milo-Mac /tmp/milo-mac -- --depth 1`, then grep `Milo Mac/MiloAPIService.swift` (REST) + `WebSocketService.swift` (WS). Once, removing `GET /api/settings/{volume-limits,volume-steps,dock-apps}` broke Milo-Mac (it polled them while the frontend used `/bulk`); it has since converged onto `/bulk` + WS and those three GETs are gone — but the rule stands for the next deletion.

**Milo-Mac's coupling is the whole wire contract** — keep all of this intact unless a Milo-Mac grep proves otherwise:
...
**External consumers — deletion is not a frontend-only call**: the REST API **and WebSocket** are also consumed by **Milo-Mac** (external macOS app, not in this repo — see *External API Clients* near the top). A route or WS event with no `frontend/src/` caller may still be live for Milo-Mac; grep the Milo-Mac repo before deleting either.
...
20. **Don't delete an HTTP route OR WS event just because `frontend/src/` has no caller.** The REST API + WebSocket have a second consumer — **Milo-Mac** (external macOS app, not in this repo; see *External API Clients*). Grep the Milo-Mac repo before removing any route or event. Removing `/api/settings/volume-limits`, `volume-steps` and `dock-apps` already broke Milo-Mac once. It couples to the full wire contract — route paths/methods, request & response keys, **and** WebSocket event types/payloads (`system`/`source`/`volume`/`equalizer`/`routing`/`settings`) — so only purely frontend code is safe to clean without the external check.
