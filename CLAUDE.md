...
**A route or WS event with no caller in `frontend/src/` is NOT necessarily dead** — Milo-Mac may consume it. The surface Milo-Mac depends on is pinned in a manifest, [backend/tests/contracts/milo_mac_contract.json](backend/tests/contracts/milo_mac_contract.json) — **the single source of truth**, no manual grep anymore. `test_milo_mac_contract.py` (runs on every `pytest`, offline) fails the build if the backend drops a REST route or WS event the manifest lists; a non-blocking weekly CI job (`check_milo_mac_freshness.py`) re-clones Milo-Mac and flags it if the manifest itself falls behind what the app consumes.

**Workflow:** the manifest is updated *consciously* when Milo-Mac evolves, not auto-grepped. If a contract test fails, either restore the route/event, or — if Milo-Mac genuinely dropped it — delete that manifest entry (and check the freshness job for what to add). Don't add compatibility shims; the appliance keeps a single code path.
