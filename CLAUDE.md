...
**Milo-Mac's coupling is the whole wire contract, not just route existence.** It also opens the WebSocket and consumes `system` / `source` / `volume` / `equalizer` / `routing` events — relying on the `full_state` envelope and the `multiroom_changed` / `volume_changed` payload fields — plus the REST request-body and response keys of the routes it calls. So WS event types/payloads and request-body field names are coupled too. **Safe to clean without the external check:** only purely frontend code (Vue components, Pinia stores, frontend Zod schemas) — Milo-Mac is a separate Swift app that imports none of it.

**Convergence option (small, future).** Exactly three routes are Milo-Mac-exclusive — `GET /api/settings/volume-limits`, `volume-steps`, `dock-apps` (every other endpoint Milo-Mac calls is already shared with Milō's own frontend). Milo-Mac can drop all three with **no new Milō surface**, mirroring how the frontend already gets these values: `volume_limits` and `dock_apps` from `GET /api/settings/bulk`, and `step_mobile_db` from the `volume/volume_changed` WS event (which already carries it — [service.py](backend/core/volume/service.py) `_broadcast_volume_state`). Once Milo-Mac migrates and is retested, the three routes get deleted here — never before (deleting them first was the §2.1 mistake).
