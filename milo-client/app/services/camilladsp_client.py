"""Async WebSocket client for the satellite's local CamillaDSP daemon.

The satellite's half of `backend/core/equalizer/camilladsp_client.py`, and it is
deliberately a parallel rather than a copy: it has no import from the backend to
make, its own logger, and one command fewer — nothing here reads the capture
rate, which only the server's status surface shows.

Replacing `pycamilladsp` removes two reach-ins this file used to need: the
library was synchronous, so every DSP call crossed a thread executor, and the
only way to stop `recv()` blocking for ever when the daemon went down was to
reach through `client._ws.sock` and set a socket timeout on a private attribute.

The protocol, captured from CamillaDSP 4.1.3 on 127.0.0.1:1234 (2026-09-14) —
the daemon the server runs, which is the same build the satellites run. A
param-less command is a bare JSON string, a parameterised one an object, and the
answer comes back under the command's own name:
...
idle connection), so an idle connection survives on the library's own keepalive;
`EqualizerService._probe_connection` still asks `GetState` on top, because a
daemon whose command loop is wedged answers pings all the same.
"""
import asyncio
import contextlib
import json
...
logger = logging.getLogger(f"{__name__}.CamillaDspClient")
